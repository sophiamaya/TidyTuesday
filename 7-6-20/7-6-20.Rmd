...
map <- ggplot(rating.map, aes(map_id = region, fill = average_rating))+
  ...
  na.value = "grey50") +
  theme_void()+
  labs(title ="Where is the CoffeE?", subtitle = "Average cup points by county", fill="Series") +
  theme(plot.title = element_text(hjust = 0.5, size = 25, color="darkgreen")) +
  theme(plot.subtitle = element_text(hjust = 0.5, face = "italic", color="darkgreen", size = 15))
...
bar <- ggplot(best_rated, aes(x= reorder(region, average_rating), y=average_rating)) +
  ...
  theme_minimal() +
  labs(subtitle = "Top Countries", fill="Series") +
  theme(plot.subtitle = element_text(hjust = 0.5, color="darkgreen", size = 15))
